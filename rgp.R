...
rgp <- function(f,fname,nlev,ylab,xlab,xlo,xhi){
...
#source("outer.R")  # creates outer framework plot for up to 3 factors
                    ...
                    #  plot locations
# outer.R
# By Russell Barton September 2025
...
outer <- function(unscaledDOE,outerd,innerd){
  ...
  resp = unscaledDOE[,(outerd+innerd+1)]
  minresp = min(resp)
  maxresp = max(resp)
  delta = 0.1
  sResp = delta + (resp-minresp)/(maxresp-minresp)
  ...
  # add lines of cube - background thin and grey if outerd = 3
  if(outerd == 3){
    segments(x0s,y0s,x1s,y1s,col="grey")
  ...
  #  background lines which are grey (no background lines for outerd = 1, 2)
  ...
  tscale = pscale*1 # this is adjustable to improve axis label appearance
    ...
    for(irow in 1:nrow(outerDOE)){
      ...
      if(innerd==0 & outerd<3){
        symbols(ploc[1]+xshift,ploc[2],
                circles=min((2/numalev),(2/numblev))*rscale*sqrt(sResp[irow]),bty="n",xaxt="n",yaxt="n",
                xlab="",ylab="", bg="grey30",fg="grey30", inches=FALSE, add=TRUE)
      } else if(innerd==0 & outerd==3){
        symbols(ploc[1]+xshift,ploc[2]+yshift,
                circles=min((2/numalev),(2/numblev),(2/numclev))*rscale*sqrt(sResp[irow]),bty="n",xaxt="n",yaxt="n",
                xlab="",ylab="", bg="grey30",fg="grey30", inches=FALSE, add=TRUE)
      }
      # if innerd > 0
      if(innerd>0){
        ...
        if(outerd==1){
          innerDOE = DOE[DOE[,1:outerd] == outerRow,((outerd+1):(d+1))]
        } else {
          innerDOE = DOE[apply(DOE[,1:outerd], 1, 
                               function(y) return(all(y == outerRow))),((outerd+1):(d+1))]
        }
 ...
 if(innerd==3) psResp=.8*spscale*rscale*sqrt(sResp)
  ...
  if(innerd ==1){
    symbols(spscale*DOExyzS[1,1:ncol(DOExyz)]+ploc[1],rep(0,ncol(DOExyz))+ploc[2],
            circles=psResp,bty="n",xaxt="n",yaxt="n",
